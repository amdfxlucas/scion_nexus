...
namespace
{
  ...
  configuration parse_args(int argc, char **argv)
  {
    ...
    int file_args_begin = 3;
    if (std::string_view(argv[3]) == "true")
    {
      config.scion = argv[3];
      file_args_begin = 4;
    }
    config.files_begin = std::next(argv, file_args_begin);
...
int main(int argc, char **argv)
{
  ...
  if (ec)
    throw std::runtime_error("CA file not found");
